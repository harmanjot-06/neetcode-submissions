class Solution {
    ...
    public boolean catchup(int pos1, int pos2, int speed1, int speed2, int target){
        ...
        float relspeed = speed1 - speed2;
        if(relspeed<=0)return false;

        float reldistance = pos2 - pos1;

        float time = reldistance/relspeed;

        float finalpos = pos1 + time*speed1;

        // System.out.println("finalpos " + finalpos + " target " + target);
